...
export async function onRequestPost(context) {
  try {
    ...
    const products = await getProducts(env.HONGTE_KV);
    const newProduct = { ...body, id: Date.now().toString(), images: body.images || [], specs: body.specs || [], featured: body.featured || false };
...
export async function onRequestPut(context) {
  try {
    ...
    if (index === -1) return jsonResponse({ error: 'Product not found' }, 404);
